...
class TSNDATAREADER(object):
    def __init__(self, name, mode, cfg):
        ...
        self.mode = mode
        self.name = name
        self.format = cfg.MODEL.format #pytorch: modality
        self.num_classes = self.get_config_from_sec('model', 'num_classes')
        self.seg_num = self.get_config_from_sec(mode, 'seg_num', self.seg_num) #pytorch: num_segments
        self.new_length = 1
        if self.format == 'RGBDiff':
    ...
    def create_reader(self):
        ...
        _reader = self._reader_creator() #todo
        def _batch_reader():
            batch_out = []
